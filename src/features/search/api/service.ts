...
import type { SearchRequest, BackendSearchResponse, SearchResponse } from './types';
...
function extractResults(response: BackendSearchResponse): SearchResult[] {
  ...
  if (response.data && typeof response.data === 'object') {
    ...
    const hasGroupedData = ['players', 'teams', 'matches', 'leagues'].some((key) =>
      Array.isArray(d[key])
    );
    ...
    if (hasGroupedData) {
      ...
      if (Array.isArray(d.players)) {
        results.push(
          ...d.players.map((item: Record<string, unknown>) => ({
      ...
      if (Array.isArray(d.teams)) {
        results.push(
          ...d.teams.map((item: Record<string, unknown>) => ({
            id: item.id as string,
      ...
      if (Array.isArray(d.matches)) {
        results.push(
          ...d.matches.map((item: Record<string, unknown>) => ({
            id: item.id as string,
            type: 'matches' as SearchType,
            title: item.name as string,
            subtitle: (item.subTitle || item.subText) as string | undefined,
            imageUrl: (item.imageUrl || (item.meta as Record<string, unknown>)?.homeLogo) as
              | string
              | undefined,
            metadata: item.meta as Record<string, unknown> | undefined,
          }))
      ...
      if (Array.isArray(d.leagues)) {
        results.push(
          ...d.leagues.map((item: Record<string, unknown>) => ({
            id: item.id as string,
...
export class SearchService {
  ...
  static async search(request: SearchRequest): Promise<SearchResponse> {
    ...
    const total =
      response.data && typeof response.data === 'object' && 'totalResults' in response.data
